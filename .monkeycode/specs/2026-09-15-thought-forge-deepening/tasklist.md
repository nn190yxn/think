...
- [x] 16.3 真机验证模型平台连通与密钥不落库（**2026-09-21**：替身端点连通、密钥只存系统凭据库、`--secret` 扫库通过；**真实密钥下的连通性仍待你配一次**）
- [x] 16.4 真机验证会诊全链路与调用审计（**2026-09-21 已做**：三场会诊、发言 51 条、各用途调用次数与结论齐；真模型下的语气与内容仍属人工项）
- [x] 16.5 真机验证蒸馏全链路（**2026-09-21 已做**：快照上跑完六阶段、过骨架确认门、技能单元 24 条、版本升到 2，检查器 V7 通过）
- [x] 16.6 真机验证搜索、网页阅读与 MCP 三类连接器及其审计（**2026-09-21 已做**：三类各真调一次、审计齐全，检查器 V13/V11 通过）
- [x] 16.7 真机验证操作系统凭据库读写与凭据引用恢复（**2026-09-21 已做**：跨进程写入与读回一致、应用解析一致、密钥未落库、库内只 1 条引用；探针 `src-tauri/src/credential_probe.rs`）
- [x] 16.8 真机验证备份创建、恢复与迁移前自动备份（**2026-09-21 已做**：坏备份被拒（修掉 G7）、好备份恢复后逐表一致、旧库 16→17 时先生成迁移前自动备份）
- [ ] 16.9 （暂缓·2026-09-21 Alex 定：先不封装签名）真机验证 NSIS 与 WiX 安装包安装、自动升级与签名
- [x] 16.10 记录真机验证结论、偏差与后续动作（**2026-09-21**：《真机验收报告》十一节 + 缺陷 G0–G7 与未做项）
