...
**不要一口气把 43 项做完。** 43 项里你只做 28 项（P20 15 项、P21 4 项、P23 5 项、设置页剩 2 项、
以及收尾同步）；**P22 补料 7 项与 P16 真机验收 8 项由 Alex 本人做**，见第五节末尾与第八节。

分三段走，每段结束**停下汇报、等人看完再往下**。每段汇报三行：做了什么 → 门禁证据（命令 + 结果尾巴）→ 哪些没验。

### 第 1 段：工程底座与名册数据

停止条件：`pnpm gate:core` 与 `pnpm gate:shell` 全绿；`tests/council.rs` 新增了 `layer_depth` 断言；
20.4 的 `master_list` 带上六题积累计数（前端类型与演示桩同步）。

### 第 2 段：点将面板（**必须等 Alex 亲手点一遍**）

停止条件：七条前端用例通过；面板能被点开、选人、留痕、取消、Esc 关闭；**Alex 认可手感后**才继续。
理由：界面好不好用是主观判断，机器判不了；一次做满二十项再返工最贵。

### 第 3 段：入口与收尾

停止条件：`pnpm gate:all` 全绿；`README.md` 规格表与当前进度、`tasklist.md` 勾选、`任务记忆.md` 登记同步完成；
逐项提交并推送。

### 贯穿三段的铁律

交付前必须有门禁证据；**没验的写"未验"，不许写"已完成"**；拿不准的停下来问 Alex，不要自行扩大改动面。
